...
- **Big Quest — "Chaos Ascendant"**: like every character, the Wizard has
  a Big Quest (visible in the quest panel). The objective is to **slay 8
  foes with Chaos Magic** over the run — kills from the ability's damaging
  bolts (fireball, lightning, spirit blast, etc.) count; knife/gun kills
  don't. The panel description tracks live progress ("Chaos kills: 3/8"),
  and the wizard shouts the tally on each qualifying kill. On completion
  it fires a **big in-run payoff** — full heal, an arcane power surge
  (Giant + Fast), a burst of XP, a 1000-money-plus-two-weapons loot stash,
  and an instant Chaos Magic recharge — and records the `Wizard_BQ`
  unlock. (Server-authoritative, matching vanilla big quests: counted on
  the host.)
...
- `Unlocks.LoadInitialUnlocks` postfix: `AddUnlock("Wizard","Agent",true)`
  ...
  runs before the postfix); also registers the `Wizard_BQ` `"BigQuest"`
  unlock for the Chaos Ascendant quest.
- `NameDB.GetName` prefix: name/description for agent + ability item
  (types "Agent", "Description", "Item"), plus the Big Quest title
  (`Wizard_BQ` → "Chaos Ascendant") and description (`D2_Wizard_BQ`,
  regenerated each query so it shows live kill progress).
...
`WizardBigQuest.cs` — the "Chaos Ascendant" Big Quest. Every player agent
has a `bigQuest` string (defaults to `agentName`, so the Wizard's is
`"Wizard"`); the game's `Quests` switch blocks have no case for it, so the
quest is driven entirely by these patches:
- `ChaosMagic.cs` tags the bolts it fires (`Bullet.cameFromWeapon =
  "ChaosMagic"`).
- `BulletHitbox.HitAftermath` postfix: when a `"ChaosMagic"`-tagged bolt
  owned by a Wizard strikes an NPC, records that victim (with the wizard
  and `Time.time`) in a `ConditionalWeakTable`.
- `Quests.AddBigQuestPoints(killer, victim, ..., "Neutralize")` postfix:
  the game calls this once per NPC kill on the server (the canonical
  per-kill hook, ungated by the killer's quest). If the victim was a
  recent Chaos-bolt victim of this wizard (within 6 s), it counts toward
  the quest. Progress lives on the wizard `Agent` in a
  `ConditionalWeakTable` (survives floor transitions, resets per run).
- On reaching 8, marks the `Wizard_BQ` unlock complete, adds the player to
  `sessionData.agentsCompletedBigQuest`, shows the `BigQuestCompleted`
  banner, and grants the in-run payoff (heal, Giant+Fast, XP via
  `skillPoints.AddPoints("KillPoints")`, `AddItem("Money",1000)` +
  `AddRandWeapon()`×2, ability recharge). All payoff steps are
  try/catch-guarded.
- Counting is gated on `gc.serverPlayer` (true in single-player), so it
